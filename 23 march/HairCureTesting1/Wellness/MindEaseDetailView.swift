...
struct MindEaseDayDetailView: View {
    ...
    private var targetMinutes: Int { store.dailyMindfulTarget }
    ...
    private var categoryBreakdown: [(title: String, icon: String, minutes: Int)] {
        ...
        for session in sessions {
            if let content = store.mindEaseCategoryContents.first(where: { $0.id == session.contentId }) {
                buckets[content.categoryId, default: 0] += session.minutesCompleted
            }
        }
        return store.mindEaseCategories.compactMap { cat in
            ...
            if sessions.isEmpty {
                ...
                VStack(spacing: 0) {
                    ForEach(Array(sessions.enumerated()), id: \.element.id) { idx, session in
                        SessionRow(session: session, store: store, purple: purple)
...
private struct SessionRow: View {
    ...
    private var contentTitle: String {
        store.mindEaseCategoryContents.first(where: { $0.id == session.contentId })?.title ?? "Session"
    }

    private var categoryIcon: String {
        guard
            let content  = store.mindEaseCategoryContents.first(where: { $0.id == session.contentId }),
            let category = store.mindEaseCategories.first(where: { $0.id == content.categoryId })
        else { return "brain.head.profile" }
        return category.cardIconName
    }

    private var mediaType: String {
        store.mindEaseCategoryContents.first(where: { $0.id == session.contentId })?.mediaType ?? "video"
    }
