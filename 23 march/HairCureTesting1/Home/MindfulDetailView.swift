    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 20) {
            HStack(alignment: .firstTextBaseline, spacing: 8) {
                ...
                Text("\(store.todaysMindfulMinutes()) min")
        ...
        .onAppear {
            ...
            segments = [
                MindfulSegment(name: "Relaxing Sounds", color: .teal, minutes: min(5, store.todaysMindfulMinutes() / 3), target: 5),
                MindfulSegment(name: "Yoga", color: .red, minutes: store.todaysMindfulMinutes() / 2, target: yogaTarget),
                MindfulSegment(name: "Meditation", color: .blue.opacity(0.5), minutes: store.todaysMindfulMinutes() / 4, target: breathTarget)
            ]
        }
    }
}
