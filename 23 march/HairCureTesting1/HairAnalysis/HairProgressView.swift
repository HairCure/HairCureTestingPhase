...
//////
//////  Navigation flow:
//////  HomeView (View Hair Progress) → HairProgressView
//////                               → HairProgressAllScansView  (See all)
//////                               → HairProgressDetailView    (View Details)
//////
////
////import SwiftUI
////
////// ─────────────────────────────────────────────────────────────
////// MARK: 1 — Journey View  (shown when "View Hair Progress" tapped on HomeView)
////// "Hair Progress" title · Hair Journey list · See all · Floating camera FAB
////// ─────────────────────────────────────────────────────────────
////
////struct HairProgressView: View {
////    @Environment(AppDataStore.self) private var store
...
////                    .buttonStyle(.plain)
////
////                    // Bottom padding so FAB doesn't obscure last card
...
//    let densityPercent: Int
//    let doneOn: String          // "05 Dec,2025"
//    let scanReport: ScanReport? // non-nil only for the real last scan
//}
//
///// Builds the entry list for a given month.
///// The real `latestScanReport` (if it falls in the requested month) is injected
///// as the first — and only real — entry; the remaining slots are placeholder
///// stubs that show "Scan not taken" in the detail view.
//func hairProgressEntries(for month: Date, store: AppDataStore) -> [HairProgressEntry] {
//    let cal = Calendar.current
//    let comps = cal.dateComponents([.year, .month], from: month)
//    guard let firstOfMonth = cal.date(from: comps),
...
//    var entries: [HairProgressEntry] = []
//
//    // ── 1. Inject the real scan report if it falls in this month ──────────
//    if let report = store.latestScanReport {
//        let scanDate = report.createdAt
//        if scanDate >= firstOfMonth && scanDate < firstOfNext {
//            entries.append(HairProgressEntry(
//                id: report.id,
//                densityPercent: Int(report.hairDensityPercent),
//                doneOn: df.string(from: scanDate),
//                scanReport: report          // ← carries full scan data
//            ))
//        }
//    }
//
//    // ── 2. Fill remaining weekly placeholder slots ─────────────────────────
//    // Offsets count backwards from the last day of the month.
//    // Skip whichever date matches the real scan so we don't duplicate.
//    let realScanDate = store.latestScanReport.map {
//        cal.startOfDay(for: $0.createdAt)
//    }
//    let weekOffsets: [Int] = [0, 7, 14, 21, 28]
//
//    for weekBack in weekOffsets {
//        guard let candidateDate = cal.date(byAdding: .day, value: -weekBack, to: firstOfNext),
//              candidateDate >= firstOfMonth
//        else { continue }
//
//        // Don't add a stub for the day that already has the real scan
//        if let realDay = realScanDate,
//           cal.isDate(candidateDate, inSameDayAs: realDay) {
//            continue
//        }
//
//        // Stub entry — no scan report attached
//        entries.append(HairProgressEntry(
//            id: UUID(),
//            densityPercent: 0,
//            doneOn: df.string(from: candidateDate),
//            scanReport: nil             // ← placeholder; detail shows "not taken"
//        ))
//    }
//
//    return entries
...
//                    // Real scan — show actual density
//                    Text("Density Percentage : \(entry.densityPercent)%")
//                        .font(.system(size: 16, weight: .semibold))
//                        .foregroundColor(.primary)
//                } else {
//                    // Placeholder — no scan taken
//                    Text("Scan not taken")
//                        .font(.system(size: 16, weight: .semibold))
//                        .foregroundColor(.secondary)
//                }
...
//// ─────────────────────────────────────────────────────────────
//// MARK: 6 — Detail View
//// Branches on whether entry.scanReport is present.
//// ─────────────────────────────────────────────────────────────
//
//struct HairProgressDetailView: View {
//    let entry: HairProgressEntry
...
//                            valueColor: densityColor(report.hairDensityPercent))
//
//                analysisRow(title: "Growth Stage",
//                            value: "Stage \(report.hairFallStage.intValue)",
//                            valueColor: stageColor(report.hairFallStage.intValue))
//
//                analysisRow(title: "Scalp Condition",
//                            value: scalpLabel(report.scalpCondition),
//                            valueColor: Color(red: 0.2, green: 0.55, blue: 0.9))
//
//                analysisRow(title: "Scan Type",   value: "Weekly scan",   valueColor: .primary)
//                analysisRow(title: "Analysis Source", value: report.analysisSource.rawValue, valueColor: .primary)
//            }
//            .padding(.horizontal, 20)
//
//            // ── Lifestyle scores card ────────────────────────────────────
...
//                    .foregroundColor(Color.hcBrown.opacity(0.40))
//            }
//
//            VStack(spacing: 8) {
//                Text("Scan Not Taken")
...
//            }
//
//            // Greyed-out placeholder rows to show what would appear
//            VStack(spacing: 10) {
...
//    }
//
//    private func analysisRow(title: String, value: String, valueColor: Color) -> some View {
//        HStack {
//            Text(title)
//                .font(.system(size: 15, weight: .medium))
//                .foregroundColor(.secondary)
...
//                    dimBar("Stress",
//                           report.stressScore,
//                           Color(red: 0.4, green: 0.72, blue: 0.35))
//                    dimBar("Diet",
//                           report.dietScore,
//                           Color(red: 0.9, green: 0.58, blue: 0.18))
//                    dimBar("Hair Care",
//                           report.hairCareScore,
//                           Color.hcBrown)
//                }
//            }
//        }
...
//        let frac: CGFloat = CGFloat(score / 10.0)
//        let c: Color = score < 5 ? .red : score < 8 ? .orange : .green
//        return ZStack {
//            Circle()
//                .stroke(c.opacity(0.18), lineWidth: 10)
//            Circle()
...
//                               → HairProgressAllScansView  (See all)
//                               → HairProgressDetailView    (View Details)
//                               → WeeklyScanView            (camera FAB — weekly)
//                               → AssessmentView            (camera FAB — monthly)
//
