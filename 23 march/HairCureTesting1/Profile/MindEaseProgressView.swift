...
struct MindEaseProgressView: View {
    ...
    private var todaySummaryCard: some View {
        let done   = store.mindfulMinutes(for: Date())
        let target = max(1, store.dailyMindfulTarget)
        let prog   = min(Double(done) / Double(target), 1.0)

        let cal        = Calendar.current
        let todayStart = cal.startOfDay(for: Date())
        let sessions   = store.mindfulSessions.filter {
        ...
        let catMins: [(name: String, mins: Int, icon: String, color: Color)] = [
            ("Meditation", sessions.filter { isMeditation($0.contentId) }.reduce(0) { $0 + $1.minutesCompleted }, "brain.head.profile", Color(red: 0.55, green: 0.32, blue: 0.12)),
            ("Yoga",       sessions.filter { isYoga($0.contentId) }.reduce(0)       { $0 + $1.minutesCompleted }, "figure.yoga",         Color(red: 0.26, green: 0.20, blue: 0.16)),
            ("Sounds",     sessions.filter { !isMeditation($0.contentId) && !isYoga($0.contentId) }.reduce(0) { $0 + $1.minutesCompleted }, "waveform", Color(red: 0.08, green: 0.30, blue: 0.30))
        ].filter { $0.mins > 0 }
    ...
    private var ringSelectorSection: some View {
        let cal    = Calendar.current
        let target = Double(max(1, store.dailyMindfulTarget))
        ...
        return VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 0) {
                ForEach(ringDates, id: \.self) { date in
                    ...
                    let mins       = store.mindfulMinutes(for: date)
    ...
    private func isMeditation(_ contentId: UUID) -> Bool {
        guard let c = store.mindEaseCategoryContents.first(where: { $0.id == contentId }),
              let cat = store.mindEaseCategories.first(where: { $0.id == c.categoryId })
        else { return false }
        return cat.title == "Meditation"
    }

    private func isYoga(_ contentId: UUID) -> Bool {
        guard let c = store.mindEaseCategoryContents.first(where: { $0.id == contentId }),
              let cat = store.mindEaseCategories.first(where: { $0.id == c.categoryId })
        else { return false }
        return cat.title == "Yoga"
    }
...
private struct SessionDetailRow: View {
    ...
    private var contentInfo: (title: String, icon: String, catName: String) {
        guard let c   = store.mindEaseCategoryContents.first(where: { $0.id == session.contentId }),
              let cat = store.mindEaseCategories.first(where: { $0.id == c.categoryId })
