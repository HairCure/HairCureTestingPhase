...
//        }
        func fillProgress(for date: Date) -> Double {
            let target   = Double(dietMateStore.totalCalorieTarget(for: date))
            guard target > 0 else { return 0 }
            let consumed = Double(dietMateStore.totalCalories(for: date))
...
//                    }
                    ...
                    ZStack {
                        ...
                        // Grey track — hide when complete so green shows fully
                        Circle()
                            .stroke(
                                progress >= 1.0 ? Color.clear : Color.gray.opacity(0.18),
                                lineWidth: 4
                            )
                            .frame(width: 32, height: 32)
                        ...
                        if progress > 0 {
                            Circle()
                                ...
                                .stroke(
                                    isFuture ? Color.gray.opacity(0.3) : color,
                                    style: StrokeStyle(lineWidth: 4, lineCap: .butt)  // .butt avoids cap overhang
                                )
                                .rotationEffect(.degrees(-90))
                                .frame(width: 32, height: 32)
                                .animation(.easeInOut(duration: 0.4), value: progress)
